# Runbook Toolkit 仕様書

## 1. 概要

AWS CLI作業手順書の作成を効率化するツールキット。

手順書は人間がCloudShell上で読みながら対話的にコマンドを実行するためのものであり、この運用は変えない。ツールキットは手順書の「書く側の労力」を削減することを目的とする。

### 1.1 設計思想

- 手順書は人間が読んで文脈を理解しながら対話的に実行するもの。スクリプトで自動実行するものではない。
- Why（なぜその作業をするか）や前提条件は省略できない。人が交代しても意味がわかる手順書を維持する。
- 操作タイプごとのテンプレートとスニペットを資産として蓄積し、新しい手順書はYAML定義だけで量産可能にする。
- 仕組みはできるだけ単純に保つ。階層・抽象・拡張ポイントは必要になってから追加する。


## 2. 2層構造

手順書を以下の2層で組織化する。

### 2.1 手順書（Runbook）

- 1つのリソース操作に対応する手順
- About、Before/After条件、使用テンプレート、パラメータ、後続手順書へのナビゲーションを定義する
- 構造は前処理（事前確認）→ 主処理（リソース操作）→ 後処理（事後確認）で統一する
- 例：「VPCを作成する」「DNS Hostnameを有効にする」

### 2.2 スニペット（Snippet）

- 1つのAWS CLIコマンドのテンプレート
- 純粋にコマンドの内容だけを持つ。役割（事前確認・実行・事後確認）は知らない
- 役割の割り当てはテンプレート側が制御する
- 例：`ec2/create-vpc.md`、`ssm/put-parameter.md`

> 補足：以前は「シナリオ（複数手順書をまとめる上位層）」を3層目として置いていたが、運用上は手順書一覧を README 等のドキュメントで示せば十分であり、層を1つ減らした。共通パラメータは §5.2 の `params_files` で扱う。


## 3. リポジトリ構造

ツールキット（テンプレート・スニペット・ジェネレータ）と、各業務固有のYAML定義・生成物は同一リポジトリに同居する。リポジトリを分けない単純構成を採る。

```
cli_runbook_maker/
├── generate.py                     # 生成ツール
├── requirements.txt
├── snippets/                       # コマンドスニペット（共通資産）
│   ├── ec2/
│   ├── s3/
│   │   └── policies/               # サブスニペット（ポリシー JSON テンプレート）
│   ├── kms/
│   │   └── policies/
│   └── ...
├── templates/                      # 手順書テンプレート（共通資産）
│   └── runbook.md.j2
├── projects/                       # プロジェクトごとの手順書定義・生成物
│   └── example/                    # サンプル兼検証用プロジェクト
│       ├── params/
│       │   └── training-common.yaml
│       ├── runbooks/
│       │   ├── 0101-create-vpc.yaml
│       │   └── 0102-modify-dns-hostname.yaml
│       └── dist/
│           └── runbooks/
│               ├── 0101-create-vpc.md
│               └── 0102-modify-dns-hostname.md
├── SPEC.md
├── TASKS.md
├── LOGS.md
├── CLAUDE.md
└── README.md
```

### 3.1 ディレクトリの役割

| ディレクトリ | 役割 |
| --- | --- |
| `templates/` | 手順書テンプレート（共通資産） |
| `snippets/` | AWS CLI コマンドスニペット（共通資産） |
| `projects/` | プロジェクトごとのディレクトリを格納する親ディレクトリ |
| `projects/<project>/runbooks/` | 業務ごとの手順書 YAML 定義 |
| `projects/<project>/params/` | 業務ごとの共通パラメータ YAML（任意） |
| `projects/<project>/dist/runbooks/` | 業務ごとの手順書 Markdown 生成物 |

`projects/example/` がサンプル兼検証用プロジェクト。実案件を追加する場合は `projects/project-a/` のように兄弟ディレクトリとして並べる。

### 3.2 テンプレート・スニペットの探索

`generate.py` と同じディレクトリ配下の `templates/` と `snippets/` のみを参照する。プロジェクト側のオーバーライド機構は持たない。テンプレートに修正を入れたい場合は、共通資産そのものを変更するか、新しいテンプレートとして追加する。

### 3.3 生成物のgit管理

生成された `.md` ファイルはgitにコミットする。生成ツールがなくても手順書として読めることを保証するため。


## 4. YAML定義フォーマット

### 4.1 手順書YAML

```yaml
runbook:
  id: "0101"
  title: "VPCを作成する"
  about: |
    VPCを作成するCLI手順書。

    本手順では、VPCを東京リージョンに作成する。
  services:
    - EC2/VPC
  duration: "5 Min"
  operation: CREATE              # 操作タイプラベル（CREATE / MODIFY / DELETE 等）

  before:
    - 作業リージョンでVPCの作成数上限に達しておらずVPCが作成可能である。
  after:
    - VPCが作成されている。

  # 使用するテンプレート（templates/<value>.md.j2 を参照）
  # 通常は汎用テンプレート "runbook" を使う
  template: runbook

  # 1.2, 1.3, ... の事前確認ブロック（順序通り展開される）
  # title は必須。description / snippet は任意
  pre_checks:
    - title: "VPC作成数の上限の確認"
      description: |
        リージョン内に作成できるVPCの数は、上限緩和していなければ5個まで。
      snippet: snippets/ec2/list-vpcs.md
    - title: "依存リソースの確認"
      description: 依存するリソースは無い。

  # 2.1 主処理に展開するスニペット
  main:
    snippet: snippets/ec2/create-vpc.md

  # 3.1, 3.2, ... の事後確認（複数可、宣言順に展開される）
  # title は必須。description / snippet は任意。pre_checks と対称
  post_checks:
    - title: "VPCの作成確認"
      description: |
        作成した VPC が一覧に出ることを確認する。
      snippet: snippets/ec2/describe-vpcs.md

  # 共通パラメータファイル（任意、複数指定可・上から順にマージ）
  params_files:
    - ../params/training-common.yaml

  # この手順書固有のパラメータ（params_files より優先）
  params: {}

  # 後続手順書へのリンク（任意、Navigation セクションに展開）
  navigation:
    next:
      title: "VPC属性を設定する"
      link: "./0102-modify-dns-hostname.md"
```

固有パラメータがある場合の例：

```yaml
runbook:
  id: "0102"
  title: "VPC属性を設定する"
  about: |
    VPCの属性値を変更し、DNS Hostnameを有効にする。
  operation: MODIFY

  before:
    - VPCが作成済みである。
  after:
    - VPCのDNS hostnamesが Enabled になっている。

  template: runbook

  pre_checks:
    - title: "VPCが作成済みであることの確認"
      snippet: snippets/ec2/find-vpc-id-by-cidr.md
    - title: "VPC属性の現状確認"
      snippet: snippets/ec2/describe-vpc-dns-attributes.md

  main:
    snippet: snippets/ec2/modify-vpc-attribute.md

  post_checks:
    - title: "VPC属性の変更確認"
      snippet: snippets/ec2/describe-vpc-attribute.md

  params_files:
    - ../params/training-common.yaml

  params:
    attribute: enableDnsHostnames
    cli_option: enable-dns-hostnames
    value: "true"            # JSON リテラルに合わせて文字列で書く（§6.7）
```

### 4.2 共通パラメータYAML

```yaml
# projects/example/params/training-common.yaml
params:
  region: ap-northeast-1
  vpc_cidr: "10.0.0.0/24"
  vpc_name: "project-dev-main-vpc"
```

トップレベルの `params` キーの直下にフラットな key-value を並べる。`params_files` で複数指定された場合は記述順にマージし、後勝ちで上書きされる。


## 5. パラメータ解決

### 5.1 マージ順序

```
runbook.params_files[0].params（ベース）
  ↓ マージ（後勝ち）
runbook.params_files[1].params
  ↓ マージ（後勝ち）
...
runbook.params（最優先）
  ↓ 注入
テンプレート（Markdown + Jinja2）
  ↓ 展開
スニペット（コマンド断片）
```

### 5.2 共通パラメータの単位

`params_files` は「複数の手順書で共有したい値」を切り出すための機構。シナリオ層を持たない代わりに、共有粒度を runbook YAML 側で明示的に選択させる。1ファイルに全部入れてもよいし、業務単位・環境単位で分割してもよい。


## 6. テンプレート仕様

### 6.1 形式

Markdown + Jinja2。テンプレートを見れば生成されるMarkdownの構造がそのまま見える。

### 6.2 汎用テンプレートの方針

テンプレートは操作タイプ × 特定リソースで増殖させない。リソースに依らない構造（前処理／主処理／後処理の骨格、§9 のセクション構成）は **単一の汎用テンプレート `templates/runbook.md.j2`** に集約する。リソース固有のコマンドはスニペット側に置き、runbook YAML が「どのスニペットを、どの役割で並べるか」を宣言する。

新しいリソースタイプ（例：S3、KMS）を追加するときは、原則としてスニペットを増やすだけで済み、テンプレート本体は変更しない。

### 6.3 テンプレートからスニペットの利用

テンプレート内で `{% include %}` を使ってスニペットを展開する。スニペットには runbook YAML の `params` がフラットにコンテキストとして渡され、Jinja2 のプレースホルダが解決される。

### 6.4 テンプレートの責務

- §9 の統一構造（前処理・主処理・後処理）の骨格を定義する
- runbook YAML の `pre_checks[]` / `main` / `post_checks[]` を宣言された順序に展開する
- マージ後の `params` から 1.1 のパラメータ表（キー・値の表）を生成する
- セクション番号（1.1, 1.2, ...）の自動採番を行う

### 6.5 スニペットの責務

- 1つの目的に対応する AWS CLI コマンドとその説明・期待される出力例を持つ
  - 1コマンドが原則だが、関連する一連のコマンド（例：「VPC を確認し ID をシェル変数に取得する」）を1スニペットにまとめてよい
- 役割（事前確認 / 主処理 / 事後確認）や実行順序は関知しない。役割は runbook YAML が指定する

### 6.6 スニペット内の変数規約（即値原則）

手順書は生成されるものなので、コマンドの値は基本的に **即値で書き出す**。読み手は手元の画面に出ているコマンドそのものが「これから流れる／流したコマンド」だと信じてよい。シェル変数で抽象化するのは、生成時には確定しない値（実行時に AWS から取得する VPC ID 等）に限る。

| 値の種類 | 例 | スニペット内の表記 | 出力例 |
| --- | --- | --- | --- |
| **静的設定**（YAML から確定） | region、vpc_cidr、vpc_name、attribute、cli_option、value | Jinja2 `{{ region }}` | `--region ap-northeast-1` |
| **実行時取得**（pre_check で AWS から取得） | VPC_ID、SUBNET_ID 等 | シェル変数 `${VPC_ID}` | `--vpc-id ${VPC_ID}` |

例（`snippets/ec2/modify-vpc-attribute.md`）：

```bash
aws ec2 modify-vpc-attribute \
    --vpc-id ${VPC_ID} \                           # 実行時取得 → シェル変数
    --{{ cli_option }} "{\"Value\":{{ value }}}" \  # 静的 → Jinja2 即値
    --region {{ region }}                          # 静的 → Jinja2 即値
```

例示出力（`結果の例`）も Jinja2 で展開してよい。実 runbook の値が結果例に反映されるとリアル化し、読み手が実際の出力と比較しやすい。

### 6.7 `file://` パターン（構造化 JSON の外出し）

CLI オプションに渡す JSON ドキュメントが大きく構造が可変な場合（ライフサイクル設定、バケットポリシー等）、スニペット内で「一時ファイル作成 → `jq` フォーマットチェック → `file://` 参照でコマンド実行」の3ステップ構成を取る。

```markdown
JSON ドキュメントを作成する。

```bash
cat << 'EOF' > /tmp/config.json
{
    "Key": "{{ value }}"
}
EOF
```

JSON のフォーマットを確認する。エラーなく整形結果が表示されれば OK。

```bash
jq . /tmp/config.json
```

設定を適用する。

```bash
aws s3api put-bucket-xxx \
    --bucket {{ bucket_name }} \
    --xxx file:///tmp/config.json \
    --region {{ region }}
```
```

`jq` によるチェックは必須とする。ヒアドキュメントの記述ミスや Jinja2 展開後の JSON 構文エラーを、CLI 実行前に検出するため。

**利点**：

- JSON が整形済みで可読性が高い
- CLI コマンドが短く保たれる
- 構造の変更がスニペットの差し替えだけで済む

**使い分け基準**：

| 条件 | 方式 |
| --- | --- |
| JSON なし、またはキーバリュー形式 | インライン（`Status=Enabled` 等） |
| JSON が小さく構造が固定 | インライン（`'{"Rules":[...]}'`） |
| JSON が大きい、または構造が可変 | `file://` パターン |

§6.5 の例外規定（「関連する一連のコマンドを1スニペットにまとめてよい」）に該当する。ファイル作成とコマンド実行は目的が一体であり、独立したスニペットに分離しない。

### 6.8 値の YAML 表記の規約

JSON リテラルとしてコマンドに埋め込まれるパラメータ（`value` 等）は、**JSON リテラルそのものに一致する文字列で書く**:

```yaml
params:
  value: "true"     # JSON で "true" と書きたいので YAML でも "true"（文字列）
  # NG: value: true   ← Python の bool になり、Jinja2 経由で "True" になる
  # NG: value: 30     ← 数値はそのまま動くが、整合性のため "30" 推奨
```

理由：YAML の bool/int をそのまま使うと、Python の str() 規則で `True`／`False` のように大文字化され、JSON リテラル（`true`／`false`）と一致しなくなる。1.1 のパラメータ表とコマンド本体での表記を一致させるためにも、文字列指定を推奨する。


## 7. 生成ツール（generate.py）

### 7.1 実行方法

```bash
# 1つの手順書を生成
python generate.py projects/example/runbooks/0101-create-vpc.yaml

# 複数指定（順序自由、まとめて生成）
python generate.py projects/example/runbooks/*.yaml
```

引数は手順書YAMLのパスのみを受け取る。テンプレート・スニペットは `generate.py` と同じディレクトリ配下から参照する（`--toolkit` 等のフラグは持たない）。

### 7.2 出力先

```
<runbook_yaml>          → <runbook_yaml.parent.parent>/dist/runbooks/<basename>.md
```

例：`projects/example/runbooks/0101-create-vpc.yaml` → `projects/example/dist/runbooks/0101-create-vpc.md`

定義（YAML）と生成物（MD）は完全に分離し、`dist/` 配下に runbooks/ の構造をミラーして配置する。

### 7.3 生成物

CloudShellで読みながら実行できる完全な手順書Markdown。フォーマットは現行の手順書と同等の品質・構造を維持する。


## 8. 操作タイプの拡充計画

手順書本体のテンプレート（汎用）は1個に固定し（§6.2）、リソース対応の拡充は **スニペット追加** で行う。スニペットは1スニペット = 1 AWS CLI コマンドのアトミック単位を保ち、複数リソースを操作したい場合はそれぞれ別の runbook YAML として書く（1 runbook = 1 リソース）。

実装済み：
- 汎用テンプレート: `templates/runbook.md.j2`
- EC2/VPC スニペット:
  - `snippets/ec2/list-vpcs.md` — VPC 一覧（事前確認向け）
  - `snippets/ec2/find-vpc-id-by-cidr.md` — CIDR で VPC を検索し、ID をシェル変数に取得
  - `snippets/ec2/describe-vpcs.md` — VPC 詳細を CIDR で確認（事後確認向け）
  - `snippets/ec2/describe-vpc-attribute.md` — VPC 属性確認（汎用、属性名はパラメータ）
  - `snippets/ec2/describe-vpc-dns-attributes.md` — DNS 関連 VPC 属性確認
  - `snippets/ec2/create-vpc.md` — VPC 作成
  - `snippets/ec2/modify-vpc-attribute.md` — VPC 属性変更（汎用、CLI オプション・値はパラメータ）
  - `snippets/ec2/describe-subnets.md` — サブネット確認（中立。VPC+CIDR でフィルタ）
  - `snippets/ec2/create-subnet.md` — サブネット作成（SUBNET_ID をシェル変数に取得）
  - `snippets/ec2/describe-internet-gateways.md` — インターネットゲートウェイ確認（中立。VPC アタッチ状態でフィルタ）
  - `snippets/ec2/create-internet-gateway.md` — インターネットゲートウェイ作成（IGW_ID をシェル変数に取得）
  - `snippets/ec2/attach-internet-gateway.md` — インターネットゲートウェイを VPC にアタッチ
  - `snippets/ec2/describe-route-tables.md` — ルートテーブル確認（中立。VPC+Name タグでフィルタ）
  - `snippets/ec2/create-route-table.md` — ルートテーブル作成（RTB_ID をシェル変数に取得）
  - `snippets/ec2/create-route.md` — ルートテーブルにルート追加
  - `snippets/ec2/associate-route-table.md` — ルートテーブルをサブネットに関連付け
  - `snippets/ec2/describe-security-groups.md` — セキュリティグループ確認（中立。VPC+グループ名でフィルタ）
  - `snippets/ec2/create-security-group.md` — セキュリティグループ作成（SG_ID をシェル変数に取得）
  - `snippets/ec2/authorize-security-group-ingress.md` — インバウンドルール追加
  - `snippets/ec2/describe-flow-logs.md` — VPC フローログ確認（中立。VPC でフィルタ）
  - `snippets/ec2/create-flow-logs-s3.md` — VPC フローログを S3 に配信
  - `snippets/ec2/create-flow-logs-cloudwatch.md` — VPC フローログを CloudWatch Logs に配信
- SSM/Parameter Store スニペット:
  - `snippets/ssm/describe-parameter.md` — 指定名のパラメータの状態を確認（中立。存在/非存在いずれの期待でも使える）
  - `snippets/ssm/put-parameter.md` — パラメータ1件を作成
  - `snippets/ssm/put-parameter-overwrite.md` — 既存パラメータの値を更新（`--overwrite`）
  - `snippets/ssm/get-parameter.md` — 指定名のパラメータの値を取得
  - `snippets/ssm/label-parameter-version.md` — パラメータの最新バージョンにラベルを付与
  - `snippets/ssm/get-parameter-history.md` — パラメータのバージョン履歴を取得（ラベル付与確認向け）
- S3 スニペット:
  - `snippets/s3/list-buckets.md` — バケット一覧（事前確認向け）
  - `snippets/s3/head-bucket.md` — バケット存在確認（中立。存在/非存在いずれの期待でも使える）
  - `snippets/s3/create-bucket.md` — バケット作成（`LocationConstraint` 付き）
  - `snippets/s3/get-bucket-versioning.md` — バージョニング設定確認
  - `snippets/s3/put-bucket-versioning.md` — バージョニング設定
  - `snippets/s3/get-bucket-encryption.md` — デフォルト暗号化設定確認（SSE-S3 / SSE-KMS 両対応）
  - `snippets/s3/put-bucket-encryption-sse-s3.md` — SSE-S3 (AES256) 暗号化設定
  - `snippets/s3/put-bucket-encryption-sse-kms.md` — SSE-KMS 暗号化設定（S3 Bucket Key 有効化含む）
  - `snippets/s3/get-bucket-lifecycle-configuration.md` — ライフサイクル設定確認
  - `snippets/s3/put-bucket-lifecycle-configuration.md` — ライフサイクル設定（`file://` パターン、§6.7）
  - `snippets/s3/get-bucket-policy.md` — バケットポリシー確認（`jq` でパース）
  - `snippets/s3/put-bucket-policy.md` — バケットポリシー設定（`file://` + サブスニペットパターン、§6.7。`{% include bucket_policy_template %}` でポリシー JSON を外部ファイルから取り込む）
  - `snippets/s3/policies/full-restriction.json.j2` — バケットポリシーのサブスニペット（4 Statement: DenyNonTLSAccess / DenyNonSpecifiedKMSEncryptedPut / DenyNonVPCEndpointAccess / DenyNonAuthorizedRoleGet）

- KMS スニペット:
  - `snippets/kms/list-keys.md` — リージョン内のキー一覧（事前確認向け）
  - `snippets/kms/create-key.md` — カスタマーマネージドキー作成（KeyId をシェル変数 KEY_ID に取得）
  - `snippets/kms/describe-key.md` — キー詳細確認（中立。`key_ref` で KeyId / エイリアスいずれも指定可）
  - `snippets/kms/list-aliases.md` — キーに設定されたエイリアス一覧（中立。設定/未設定の両例）
  - `snippets/kms/create-alias.md` — キーにエイリアスを設定
  - `snippets/kms/get-key-policy.md` — キーポリシー確認（`--query Policy --output text | jq .` でパース）
  - `snippets/kms/put-key-policy.md` — キーポリシー設定（`file://` + サブスニペットパターン、§6.7。`{% include key_policy_template %}` でポリシー JSON を外部ファイルから取り込む）
  - `snippets/kms/policies/admin-usage.json.j2` — キーポリシーのサブスニペット（3 Statement: EnableRootAccountAccess / AllowKeyAdministration / AllowKeyUsage）

- CloudFormation スニペット:
  - `snippets/cloudformation/list-stacks.md` — アクティブなスタック一覧（事前確認向け）
  - `snippets/cloudformation/describe-stacks.md` — スタック詳細確認（中立。存在/非存在の両例）
  - `snippets/cloudformation/create-stack.md` — スタック作成（S3 上のテンプレートを `--template-url` で参照。`validate-template` + `create-stack` + `wait`）
  - `snippets/cloudformation/describe-stack-events.md` — スタックイベント確認（直近10件、`--output table`）
  - `snippets/cloudformation/create-change-set.md` — Change Set 作成（`--use-previous-template` + `wait`）
  - `snippets/cloudformation/describe-change-set.md` — Change Set 詳細確認（中立。存在/非存在の両例）
  - `snippets/cloudformation/execute-change-set.md` — Change Set 実行（`execute-change-set` + `wait stack-update-complete`）
  - `snippets/cloudformation/delete-stack.md` — スタック削除（`delete-stack` + `wait stack-delete-complete`）

- DynamoDB スニペット:
  - `snippets/dynamodb/list-tables.md` — リージョン内のテーブル一覧（事前確認向け）
  - `snippets/dynamodb/describe-table.md` — テーブル詳細確認（中立。存在/非存在の両例）
  - `snippets/dynamodb/create-table.md` — テーブル作成（インライン + `wait table-exists`）
  - `snippets/dynamodb/update-table-gsi.md` — GSI 追加（`file://` パターン + `wait table-exists`）
  - `snippets/dynamodb/describe-time-to-live.md` — TTL 設定確認（中立。有効/無効の両例）
  - `snippets/dynamodb/update-time-to-live.md` — TTL 有効化（インライン）


## 9. 生成されるMarkdownの構造

生成物は以下の統一構造に従う。

```
# [XXXX] タイトル

## About
## When: 作業の条件
### Before: 事前前提条件
### After: 作業終了状況
## How: 以下は作業手順
### 1. 前処理
#### 1.1 処理パラメータ              ← パラメータ表（キー・値）
#### 1.2 事前条件Nの確認             ← pre_checks[] が 1.2, 1.3, ... と展開
### 2. 主処理
#### 2.1 リソースの操作 (CREATE/MODIFY/DELETE)
### 3. 後処理
#### 3.X [post_check の title]       ← post_checks[] が 3.1, 3.2, ... と展開
#### Navigation                      ← navigation.next がある場合のみ
# EOD
```

### 9.1 1.1 処理パラメータ

マージ済みの `params` から自動生成される表。例：

```
| キー | 値 |
| --- | --- |
| region | `ap-northeast-1` |
| vpc_cidr | `10.0.0.0/24` |
| vpc_name | `project-dev-main-vpc` |
```

旧仕様にあったシェル変数代入ブロック（`AWS_REGION="ap-northeast-1"` 等）は廃止。コマンドは即値で出力されるため、シェル変数で抽象化する必要がない（§6.6）。
